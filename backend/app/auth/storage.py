...
_REMOVE_USER_ROLE_MIGRATION = "remove-user-role-v4"
...
def _remove_user_role(conn: sqlite3.Connection) -> None:
    columns = {row[1] for row in conn.execute("PRAGMA table_info(users)").fetchall()}
    if "role" not in columns:
        return

    role_reference = re.compile(r"(?<![A-Za-z0-9_])role(?![A-Za-z0-9_])", re.IGNORECASE)
    dependent_objects = conn.execute("""
        ...
        WHERE tbl_name = 'users'
          ...
          AND sql IS NOT NULL
    """).fetchall()
    for object_type, name, sql in dependent_objects:
        if role_reference.search(sql):
            conn.execute(
                f"DROP {object_type.upper()} IF EXISTS {_quoted_identifier(name)}"
            )
...
def init_users_db() -> None:
    with closing(_get_conn()) as conn, conn:
        ...
        """)
        _remove_user_role(conn)
        now = datetime.now(timezone.utc).strftime("%Y-%m-%d %H:%M:%S")
        conn.execute(
            "INSERT OR IGNORE INTO auth_schema_migrations VALUES (?, ?)",
            (_REMOVE_USER_ROLE_MIGRATION, now),
        )
        conn.execute("""
...
def _ensure_default_organization(conn: sqlite3.Connection, user_id: str, now: str) -> str:
    ...
    if existing:
        ...
        conn.execute(
            ...
            "VALUES (?, ?, ?, ?, 0, ?)",
            (organization_id, f"{identity}'s organization", user_id, user_id, now),
        )
    conn.execute(
...
def list_organization_members(user_id: str, organization_id: str) -> list[sqlite3.Row]:
    ...
    with closing(_get_conn()) as conn:
        if _organization_details(conn, user_id, organization_id) is None:
            raise OrganizationNotFound("Organization not found")
        return conn.execute("""
            SELECT u.id AS user_id, u.username, u.email, u.nickname, u.avatar_url, m.role, m.created_at AS joined_at
...
def delete_organization(user_id: str, organization_id: str) -> list[str]:
    init_users_db()
    ...
    with closing(_get_conn()) as conn, conn:
        ...
        if organization is None:
            raise OrganizationNotFound("Organization not found")
        if organization["role"] != "owner":
            raise OrganizationPermissionDenied("Only organization owners can delete organizations")
        if organization["is_default"]:
            raise OrganizationPermissionDenied("Default organizations cannot be deleted")
        ...
        if "insights" in tables:
            if "insight_sources" in tables:
                media_paths.extend(
                    row["source_file_path"]
                    for row in conn.execute("""
                        SELECT source.source_file_path
                        FROM insight_sources source
                        JOIN insights insight ON insight.id = source.insight_id
                        WHERE insight.organization_id = ? AND source.source_file_path != ''
                    """, (organization_id,)).fetchall()
                )
                conn.execute("""
                    DELETE FROM insight_sources
                    WHERE insight_id IN (
                        SELECT id FROM insights WHERE organization_id = ?
                    )
                """, (organization_id,))
            media_paths.extend(
                row["source_file_path"]
                for row in conn.execute(
                    "SELECT source_file_path FROM insights "
                    "WHERE organization_id = ? AND source_file_path != ''",
                    (organization_id,),
                ).fetchall()
            )
        if "cases" in tables:
            for row in conn.execute(
                "SELECT video_url, image_urls FROM cases WHERE organization_id = ?",
                (organization_id,),
            ).fetchall():
                if row["video_url"]:
                    media_paths.append(row["video_url"])
                try:
                    media_paths.extend(json.loads(row["image_urls"] or "[]"))
                except (json.JSONDecodeError, TypeError):
                    pass
        if "content_projects" in tables and "project_memberships" in tables:
            conn.execute("""
                DELETE FROM project_memberships
                WHERE project_id IN (
                    SELECT id FROM content_projects WHERE organization_id = ?
                )
            """, (organization_id,))
        ...
        affected_users = conn.execute(
            "SELECT user_id FROM user_organization_preferences WHERE organization_id = ?",
            (organization_id,),
        ).fetchall()
        for preference in affected_users:
            fallback = conn.execute(
                "SELECT id FROM organizations WHERE default_for_user_id = ? AND id != ?",
                (preference["user_id"], organization_id),
            ).fetchone()
            if fallback:
                conn.execute(
                    "UPDATE user_organization_preferences "
                    "SET organization_id = ?, updated_at = ? WHERE user_id = ?",
                    (fallback["id"], now, preference["user_id"]),
                )
            else:
                conn.execute(
                    "DELETE FROM user_organization_preferences WHERE user_id = ?",
                    (preference["user_id"],),
                )
        conn.execute(
...
def _organization_manager_access(
    conn: sqlite3.Connection, user_id: str, organization_id: str,
) -> sqlite3.Row:
    organization = _organization_details(conn, user_id, organization_id)
    if organization is None:
        raise OrganizationNotFound("Organization not found")
    if organization["role"] not in {"owner", "admin"}:
        raise OrganizationPermissionDenied("Only organization owners and administrators can manage members")
    return organization
...
def invite_organization_member(
    ...
    with closing(_get_conn()) as conn, conn:
        conn.execute("BEGIN IMMEDIATE")
        organization = _organization_manager_access(conn, user_id, organization_id)
        if role == "admin" and organization["role"] != "owner":
...
def update_organization_member_role(
    ...
    with closing(_get_conn()) as conn, conn:
        ...
        if member_user_id == user_id:
            raise OrganizationPermissionDenied("You cannot change your own organization role")
        ...
        if organization["role"] != "owner" and member["role"] == "admin":
            raise OrganizationPermissionDenied(
                "Only the organization owner can manage organization administrators"
            )
        conn.execute(
...
def remove_organization_member(
    user_id: str, organization_id: str, member_user_id: str,
) -> None:
    init_users_db()
    with closing(_get_conn()) as conn, conn:
        conn.execute("BEGIN IMMEDIATE")
        ...
        if member is None:
            raise OrganizationMemberNotFound("Organization member not found")
        if member_user_id == user_id:
            raise OrganizationPermissionDenied("You cannot remove yourself from the organization")
        if member["owner_id"] == member_user_id or member["role"] == "owner":
            raise OrganizationPermissionDenied("The organization owner cannot be removed")
        if organization["role"] != "owner" and member["role"] == "admin":
            raise OrganizationPermissionDenied(
                "Only the organization owner can remove organization administrators"
            )
        if conn.execute(
            "SELECT 1 FROM sqlite_master WHERE type = 'table' AND name = 'project_memberships'",
        ).fetchone():
            owned_projects = conn.execute("""
                SELECT pm.project_id, o.owner_id
                FROM project_memberships pm
                JOIN content_projects p ON p.id = pm.project_id
                JOIN organizations o ON o.id = p.organization_id
                WHERE pm.user_id = ? AND pm.role = 'owner' AND p.organization_id = ?
            """, (member_user_id, organization_id)).fetchall()
            now = datetime.now(timezone.utc).strftime("%Y-%m-%d %H:%M:%S")
        ...
        preference = conn.execute(
            "SELECT organization_id FROM user_organization_preferences WHERE user_id = ?",
            (member_user_id,),
        ).fetchone()
        if preference and preference["organization_id"] == organization_id:
            default = conn.execute(
                "SELECT id FROM organizations WHERE default_for_user_id = ?",
                (member_user_id,),
            ).fetchone()
            if default:
                conn.execute(
                    "UPDATE user_organization_preferences SET organization_id = ?, updated_at = ? "
                    "WHERE user_id = ?",
                    (default["id"], datetime.now(timezone.utc).strftime("%Y-%m-%d %H:%M:%S"), member_user_id),
                )
