...
from app.engines.publishing.models import AccountMemory, ContentProject, PublishMetric, PublishReview, PublishTask, ReviewConclusion, SocialAccount
...
def init_db() -> None:
    ...
    if "media_assets" not in cols:
        conn.execute("ALTER TABLE content_projects ADD COLUMN media_assets TEXT DEFAULT '[]'")
    conn.execute("""
...
def create_project_from_session(
    ...
    conn.execute(
        ...
        """,
        (
            project_id, user_id, title or session.title or "未命名内容项目", xhs_account,
...
def create_manual_project(
    ...
    conn.execute(
        ...
        INSERT INTO content_projects (
            id, user_id, title, xhs_account, source_session_id, source_card_id,
            content_type, platform_hint, cards_snapshot, final_snapshot, notes,
            status, created_at, updated_at, media_assets
        ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
        """,
        (
            project_id, user_id, title or str(snapshot.get("title") or "Manual content project"),
            xhs_account, "", "", content_type, platform_hint, "[]", _json(snapshot),
            notes, "active", now, now, _json(media_assets or []),
        ),
    )
    conn.commit()
    conn.close()
...
def list_projects(user_id: str, xhs_account: str = "") -> list[ContentProject]:
    ...
    if xhs_account:
        rows = conn.execute(
            "SELECT * FROM content_projects WHERE organization_id = ? "
            "AND xhs_account = ? ORDER BY updated_at DESC, id DESC LIMIT 200",
            (organization_id, xhs_account),
        ).fetchall()
    else:
        rows = conn.execute(
            "SELECT * FROM content_projects WHERE organization_id = ? "
...
def add_project_media(project_id: str, media_assets: list[dict[str, Any]]) -> ContentProject | None:
    init_db()
    current = get_project(project_id)
    if not current:
        return None
    assets = [*current.media_assets, *media_assets]
    conn = _get_conn()
    conn.execute(
        "UPDATE content_projects SET media_assets = ?, updated_at = ? WHERE id = ?",
        (_json(assets), _now(), project_id),
    )
    conn.commit()
    conn.close()
...
def remove_project_media(project_id: str, media_id: str) -> ContentProject | None:
    init_db()
    current = get_project(project_id)
    if not current:
        return None
    assets = [asset for asset in current.media_assets if asset.get("id") != media_id]
    conn = _get_conn()
    conn.execute(
        "UPDATE content_projects SET media_assets = ?, updated_at = ? WHERE id = ?",
        (_json(assets), _now(), project_id),
...
def create_task_from_project(
    ...
    task_id = uuid.uuid4().hex[:12]
    selected = selected_version_ids or {}
    snapshot = {**(project.final_snapshot or {}), "media_assets": project.media_assets}
    conn = _get_conn()
    conn.execute(
        """
        INSERT INTO publish_tasks (
...
def list_tasks(user_id: str, status: str = "") -> list[PublishTask]:
    init_db()
    conn = _get_conn()
    organization_id = resolve_user_organization_id(conn, user_id)
    if status:
        rows = conn.execute(
            "SELECT * FROM publish_tasks WHERE organization_id = ? "
            "AND status = ? ORDER BY updated_at DESC, id DESC LIMIT 200",
...
def find_social_account(user_id: str, platform: str, account_name: str) -> SocialAccount | None:
    init_db()
    return _find_social_account(user_id, platform, account_name)


def _find_social_account(user_id: str, platform: str, account_name: str) -> SocialAccount | None:
    conn = _get_conn()
    organization_id = resolve_user_organization_id(conn, user_id)
    row = conn.execute(
        "SELECT * FROM social_accounts WHERE organization_id = ? "
...
def list_account_memories(user_id: str, platform: str = "") -> list[AccountMemory]:
    init_db()
    conn = _get_conn()
    organization_id = resolve_user_organization_id(conn, user_id)
    if platform:
...
def _row_to_project(row: sqlite3.Row) -> ContentProject:
    ...
    return ContentProject(
        ...
        media_assets=_load_json(row["media_assets"], []),
        status=row["status"] or "active", created_at=row["created_at"], updated_at=row["updated_at"],
    )
