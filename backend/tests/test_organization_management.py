...
class OrganizationManagementTests(unittest.TestCase):
    ...
    def test_new_default_name_and_active_preference(self):
        ...
        self.assertEqual(default["name"], "owner's organization")
    ...
    def test_missing_authentication_is_rejected(self):
        ...
        for method, path, body in (
            ...
            ("DELETE", f"/api/v1/organizations/{organization['id']}", None),
            ...
            ("DELETE", f"/api/v1/organizations/{organization['id']}/members/{self.other['id']}", None),
    ...
    def test_owner_can_delete_custom_organization_and_members_fall_back_to_default(self):
        organization = self.create()
        with sqlite3.connect(self.db_path) as conn:
            conn.execute(
                "INSERT INTO organization_memberships VALUES (?, ?, 'member', '2026-01-01')",
        ...
        storage.switch_organization(self.owner["id"], organization["id"])
        ...
        self.assertEqual(response.status_code, 200)
        with self.assertRaises(storage.OrganizationNotFound):
            storage.get_organization(self.owner["id"], organization["id"])
        self.assertEqual(
            storage.get_current_organization(self.owner["id"])["id"],
            storage.get_default_organization(self.owner["id"])["id"],
        )
        self.assertEqual(
            storage.get_current_organization(self.other["id"])["id"],
            storage.get_default_organization(self.other["id"])["id"],
        )

    def test_only_owner_can_delete_organization_and_default_is_protected(self):
        organization = self.create()
        with sqlite3.connect(self.db_path) as conn:
            conn.execute(
        ...
        protected = self.client.delete(
            f"/api/v1/organizations/{default['id']}", headers=self.headers,
        )
        self.assertEqual(protected.status_code, 403)
        self.assertEqual(
            protected.json()["detail"], "Default organizations cannot be deleted",
        )

    def test_delete_organization_removes_scoped_and_dependent_rows(self):
        organization = self.create()
        with sqlite3.connect(self.db_path) as conn:
            conn.row_factory = sqlite3.Row
            conn.execute("CREATE TABLE content_projects (id TEXT PRIMARY KEY, organization_id TEXT)")
            conn.execute("CREATE TABLE project_memberships (project_id TEXT, user_id TEXT)")
            conn.execute("CREATE TABLE insights (id TEXT PRIMARY KEY, organization_id TEXT, source_file_path TEXT)")
            conn.execute("CREATE TABLE insight_sources (insight_id TEXT, source_file_path TEXT)")
            conn.execute(
                "INSERT INTO content_projects VALUES ('project-1', ?)", (organization["id"],),
            )
            conn.execute("INSERT INTO project_memberships VALUES ('project-1', ?)", (self.owner["id"],))
            conn.execute(
                "INSERT INTO insights VALUES ('insight-1', ?, '')", (organization["id"],),
            )
            conn.execute("INSERT INTO insight_sources VALUES ('insight-1', '')")
        ...
        self.assertEqual(response.status_code, 200)
        with sqlite3.connect(self.db_path) as conn:
            for table in ("content_projects", "project_memberships", "insights", "insight_sources"):
    ...
    def test_member_can_switch_but_cannot_rename(self):
        organization = self.create()
        with sqlite3.connect(self.db_path) as conn:
    ...
    def test_member_can_open_organization_detail(self):
        ...
        self.assertEqual(
            [member["email"] for member in detail["members"]],
            ["", "other@example.com"],
        )
    ...
    def test_admin_can_invite_change_and_remove_other_members(self):
        organization = self.create()
        with sqlite3.connect(self.db_path) as conn:
            conn.execute(
                "INSERT INTO organization_memberships VALUES (?, ?, 'admin', '2026-01-01')",
        ...
        admin_candidate = storage.create_user("admin-user", "admin-user@example.com", "test-hash")
        invite_path = f"/api/v1/organizations/{organization['id']}/members"
        admin_headers = self.headers_for(self.other["id"])
        self.assertEqual(self.client.post(
            invite_path,
            headers=admin_headers,
            json={"email": admin_candidate["email"], "role": "admin"},
        ).status_code, 403)
        self.assertEqual(
            self.client.post(
        ...
        promoted = self.client.patch(
            role_path, headers=admin_headers, json={"role": "admin"},
        )
        self.assertEqual(promoted.status_code, 200)
        self.assertEqual(promoted.json()["data"]["role"], "admin")
        self.assertEqual(self.client.delete(role_path, headers=admin_headers).status_code, 403)
        self.assertEqual(self.client.patch(
            role_path, headers=admin_headers, json={"role": "member"},
        ).status_code, 403)
        detail = self.client.get(
            f"/api/v1/organizations/{organization['id']}", headers=self.headers,
        ).json()["data"]
        self.assertIn(invited["id"], [member["user_id"] for member in detail["members"]])
        self.assertEqual(self.client.patch(
            role_path, headers=self.headers, json={"role": "member"},
        ).status_code, 200)
        self.assertEqual(self.client.delete(role_path, headers=admin_headers).status_code, 200)
        owner_path = f"{invite_path}/{self.owner['id']}"
        self.assertEqual(
            self.client.patch(owner_path, headers=admin_headers, json={"role": "member"}).status_code,
            403,
        )
        self.assertEqual(
            self.client.delete(owner_path, headers=admin_headers).status_code,
            403,
        )
        admin_path = f"{invite_path}/{self.other['id']}"
        self.assertEqual(
            self.client.patch(admin_path, headers=admin_headers, json={"role": "member"}).status_code,
            403,
        )
        self.assertEqual(
            self.client.delete(admin_path, headers=admin_headers).status_code,
            403,
        )
