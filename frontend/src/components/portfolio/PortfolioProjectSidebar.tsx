...
const STORAGE_KEY = "amp-portfolio-sidebar-collapsed";

interface PortfolioProjectSidebarProps {
  projects: ContentProject[];
  selectedProjectId?: string;
}

export default function PortfolioProjectSidebar({
  projects,
  selectedProjectId,
}: PortfolioProjectSidebarProps) {
  const { t } = useI18n();
  ...
  return (
    <aside
      ...
      aria-label={t("作品项目筛选", "Portfolio project filter")}
    >
      <div className="amp-project-quick-sidebar-header">
        ...
        <button
          type="button"
          onClick={toggle}
          aria-label={collapsed ? t("展开作品侧栏", "Expand portfolio sidebar") : t("收起作品侧栏", "Collapse portfolio sidebar")}
        >
      ...
      <nav className="amp-project-quick-nav">
        ...
        >
          <span className="amp-project-quick-icon"><InlineIcon name="briefcase" /></span>
          <span className="amp-project-quick-label">{t("全部作品", "All work")}</span>
        </Link>
