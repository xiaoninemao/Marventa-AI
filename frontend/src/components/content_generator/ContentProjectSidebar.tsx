...
const STORAGE_KEY = "amp-content-sidebar-collapsed";

interface ContentProjectSidebarProps {
  projects: ContentProject[];
  selectedProjectId?: string;
}

export default function ContentProjectSidebar({
  projects,
  selectedProjectId,
}: ContentProjectSidebarProps) {
  const { t } = useI18n();
  ...
  return (
    <aside
      ...
      aria-label={t("智能创作项目筛选", "Content project filter")}
    >
      <div className="amp-project-quick-sidebar-header">
        ...
        <button
          type="button"
          onClick={toggle}
          aria-label={collapsed ? t("展开创作侧栏", "Expand content sidebar") : t("收起创作侧栏", "Collapse content sidebar")}
        >
      ...
      <nav className="amp-project-quick-nav">
        ...
        >
          <span className="amp-project-quick-icon"><InlineIcon name="edit" /></span>
          <span className="amp-project-quick-label">{t("全部创作", "All creations")}</span>
        </Link>
        ...
        <ProjectQuickSearchList projects={projects} renderProject={(project) => (
          <Link
            key={project.id}
            href={`/content_generator?project=${encodeURIComponent(project.id)}`}
