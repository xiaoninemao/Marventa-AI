  ...
  return (
    <div ref={menu_ref} className="relative shrink-0">
      <button
        ...
        className="flex items-center gap-2 hover:opacity-80 transition-opacity min-h-[36px]"
      >
        {user.avatar_url ? (
          <img
            src={user.avatar_url}
        ...
        <span className={`w-8 h-8 rounded-full flex items-center justify-center text-white text-xs font-bold ${colors[color_index]} ${user.avatar_url ? "hidden" : ""}`}>
          {initial}
        </span>
