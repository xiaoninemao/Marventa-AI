...
export default function CaseDetailPage() {
  ...
  return (
    <div className="min-h-screen bg-slate-50 dark:bg-slate-950">
      {/* Toast */}
      {toast && (
        <div className="fixed top-16 left-1/2 -translate-x-1/2 z-50 px-4 py-2.5 rounded-xl bg-zinc-900 dark:bg-zinc-100 text-white dark:text-zinc-900 text-sm font-medium shadow-xl animate-in fade-in slide-in-from-top-2">
          {toast}
        </div>
      )}
      <div className="max-w-4xl mx-auto px-6 py-8">
        ...
        <div className="flex items-center gap-2 mb-4 flex-wrap">
          <span className={`text-xs px-2 py-0.5 rounded font-medium ${
            ...
            {case_.content_type === "video" ? t("短视频", "Short video") : t("图文", "Image post")}
          </span>
          {case_.source && (
            <span className="text-xs px-2 py-0.5 rounded font-medium bg-sky-100 text-sky-700 dark:bg-sky-950 dark:text-sky-300">
              {casePlatformLabel(case_.source, t)}
            </span>
          )}
          <span className={`text-xs px-2 py-0.5 rounded font-medium ${
            case_.is_public
            ...
            {case_.is_public ? t("公开", "Public") : t("个人", "Private")}
          </span>
          {case_.is_public && (
            <span className={`text-xs px-2 py-0.5 rounded font-medium ${
              case_.category === "agency"
                ? "bg-indigo-100 text-indigo-700 dark:bg-indigo-950 dark:text-indigo-300"
                : "bg-rose-100 text-rose-700 dark:bg-rose-950 dark:text-rose-300"
            }`}>
              {case_.category === "agency" ? t("企业定制", "Agency cases") : t("行业精选", "Curated cases")}
            </span>
          )}
        </div>
        ...
        <div className="flex items-center gap-4 mb-4">
          ...
          {user && !is_admin && case_.category === "curated" && (
            <button
